...
void DataInit() {
	...
	for (int i = 0; i < 256; i++) fscanf(fp, "%f\n", &TotalSave[i]);//从文件中读取数据到内存中
	int s = SaveNumbers();
	for (int i = 0; i < 13; i++) Sheet[i] = TotalSave[s - i];//列表的等于所有的最后12个成绩
	fclose(fp);
	return;
}
void TimeSave(float inputnum) {
	FILE* fp1 = fopen("Saves.txt", "w");
	if (!fp1) { return; }
...
void DataCalculate() {
	...
	for (int i = 1, flag = 0; i < 13; i++) {
		if (Sheet[i] > 0.1 && Sheet[i] < 100)flag++;
...
void Delete() {
	...
	for (int i = 0; i < 13; i++) Sheet[i] = TotalSave[SaveNumbers() - i];
}